...
void Solver::setRhoGaussian(double cz, double cr, double varz, double varr)
{
    ...
    for (int i = 0; i < nz; i++)
    {
        for (int j = 0; j < nr; j++)
        {
            ...
            rho[i][j] = gaussFunc(z, r, cz, cr, varz, varr);
        }
    }
}

double Solver::gaussFunc(double z, double r, double cz, double cr, double varz, double varr)
{
...
{
    ofstream gout("rho_negatif_silinder.csv"); // anggap file input (rapat muatan)
    ofstream fout("phi_negatif_silinder.csv"); // file output (distribusi tegangan)
